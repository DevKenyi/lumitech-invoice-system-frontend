...
const PLANS = [
  {
    label: "Essential",
    price: "₦9,900",
    desc: "Up to 50 clients. Invoicing, expenses, capital tracking, financial reports.",
    highlight: false,
    features: ["Invoicing & payments", "Expense tracking", "Financial reports", "Capital tracking", "Up to 50 clients", "Email reminders"],
  },
  {
    label: "Business",
    price: "₦24,900",
    desc: "Unlimited clients, multi-user access, full accounting suite.",
    highlight: false,
    features: ["Everything in Essential", "Unlimited clients", "Multi-user access", "Chart of accounts & journal entries", "Bank reconciliation", "Fixed assets & depreciation"],
  },
  {
    label: "Accountant Pro",
    price: "₦59,900",
    desc: "Everything your firm needs to serve multiple clients professionally.",
    highlight: true,
    badge: "⭐ Best for Accountants",
    features: [
      ...
      <section className="py-24 bg-slate-50 border-y border-slate-100">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 items-stretch">
            {PLANS.map(plan => (
              plan.highlight ? (
                <div key={plan.label} className="relative bg-gradient-to-br from-blue-600 via-blue-700 to-indigo-700 rounded-2xl p-6 pt-8 shadow-2xl shadow-blue-600/40 flex flex-col sm:-mt-4 sm:-mb-4">
                  <div className="absolute -top-4 left-1/2 -translate-x-1/2 whitespace-nowrap">
                    <span className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-white text-blue-700 text-xs font-extrabold rounded-full shadow-lg border border-blue-100">{plan.badge}</span>
                  </div>
                  <div className="inline-flex items-center px-3 py-1 bg-white/20 text-white text-xs font-bold rounded-full mb-5 self-start tracking-widest uppercase">{plan.label}</div>
                  <div className="mb-1">
                    <span className="text-4xl font-extrabold text-white">{plan.price}</span>
                    <span className="text-blue-200 text-sm ml-1">/month</span>
                  </div>
                  <p className="text-blue-200 text-sm mb-5">{plan.desc}</p>
                  ...
                  <div className="mt-auto">
                    ...
                    <p className="text-center text-xs text-blue-300 mt-2">Then {plan.price}/month. Cancel anytime.</p>
                  </div>
                </div>
              ) : (
                <div key={plan.label} className="bg-white rounded-2xl border-2 border-slate-200 p-6 flex flex-col">
                  <div className="inline-flex items-center px-3 py-1 bg-slate-100 text-slate-600 text-xs font-bold rounded-full mb-5 self-start border border-slate-200 tracking-widest uppercase">{plan.label}</div>
                  <div className="mb-1">
                  ...
                  <div className="mt-auto">
                    ...
                    <p className="text-center text-xs text-slate-400 mt-2">Then {plan.price}/month. Cancel anytime.</p>
                  </div>
                </div>
              )
            ))}
