...
const API = import.meta.env.VITE_API_URL || "https://api.lumitechsystems.com";
...
export default function FeePortal() {
  ...
  const load = useCallback(async () => {
    try {
      const res = await pub.get(`/api/public/fees/${token}`);
      setData(res.data);
      setNotified(res.data.hasNotified);
    } catch (e) {
      setError(e.response?.data?.message || "Could not load your fee details. Please check your link.");
    } finally {
      setLoading(false);
    }
  }, [token]);
  ...
  useEffect(() => {
    ...
    if (txRef && status === "successful" && transactionId) {
      ...
      pub.post(`/api/public/fees/${token}/verify-flutterwave?transactionId=${transactionId}`)
  ...
  async function payFlutterwave() {
    setFlwLoading(true);
    try {
      const res = await pub.post(`/api/public/fees/${token}/flutterwave-link`);
  ...
  async function notifyUba() {
    setNotifying(true);
    try {
      await pub.post(`/api/public/fees/${token}/notify-uba`);
      setNotified(true);
    } catch (e) {
  ...
  if (error) return (
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6">
      <div className="text-center max-w-sm">
        ...
        <p className="text-slate-400 text-sm mt-2">If you believe this is an error, contact your LumiLedger account manager.</p>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="bg-blue-700 text-white px-6 py-5">
        <div className="max-w-2xl mx-auto">
          <p className="text-blue-200 text-sm font-medium mb-1">LumiLedger</p>
          <h1 className="text-xl font-bold">{data.orgName}</h1>
          <p className="text-blue-100 text-sm mt-0.5">Convenience Fee Statement</p>
        </div>
      </div>
